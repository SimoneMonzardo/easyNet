        ...
        >
          ...
          <div class="relative m-2">
            ...
            <input
              ...
              type="text"
              v-model="input"
              id="table-search-users-followers"
        ...
        <div class="p-6 space-y-6 h-96 overflow-y-auto mb-5">
          <div v-if="pending">
            <div
              v-for="index in 4"
              :key="index"
              class="animate-pulse flex space-x-4 mb-3"
            >
                ...
                >
                  ...
                  >
                    ...
                    <div class="pl-3">
                      ...
                      <div v-else class="font-normal text-gray-500">
                        {{ user.companyName }}
                      </div>
                    </div>
                  </th>
...
export default {
  ...
  async setup(props) {
    const { data: users, pending } = await useFetch(
      `https://progettoeasynet.azurewebsites.net/User/GetUserFollowers?userName=${props.followedBy}`,
      {
        lazy: true,
        server: false,
        method: "GET",
  ...
  methods: {
    async unfollow(user) {
      await useFetch(
        `https://progettoeasynet.azurewebsites.net/User/Unfollow?userName=${user.username}`,
        {
          method: "POST",
    ...
    async follow(user) {
      await useFetch(
        `https://progettoeasynet.azurewebsites.net/User/Follow?userName=${user.username}`,
        {
          method: "POST",
          body: JSON.stringify({ userName: user.username }),
          headers: {
