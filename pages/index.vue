<template>
  ...
  <div
    class="max-h-[calc(100vh-4rem)] w-screen grid grid-cols-[repeat(14,_minmax(0,_1fr))] grid-rows-[repeat(14,_minmax(0,_1fr))]">
    <button class="relative" @click="previousPost" name="previous-post">
      <div class="block triangle drop-shadow-lg"></div>
      <ChevronDoubleLeftIcon
        class="absolute inset-2 h-10 w-10 hover:w-11 hover:h-11 hover:inset-1.5 text-violet-600 rotate-45 bg-transparent" />
    </button>

    <div v-if="pending || data.status !== 200"
      class="post-base col-start-3 col-end-13 row-start-3 row-end-[13] h-full flex flex-col justify-center">
      <div role="status" class="space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center">
        <div class="flex items-center justify-center w-full h-96 bg-gray-300 rounded dark:bg-gray-700">
          <svg class="w-96 h-96 text-gray-200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" fill="currentColor"
            viewBox="0 0 640 512">
            <path
    ...
    <PostsFeedSection v-else id="post-feed" :post="data.posts[data.activePost]"
      class="col-start-2 sm:col-start-3 col-end-[14] sm:col-end-[13] row-start-2 sm:row-start-3 row-end-[14] sm:row-end-[13]" />
  ...
  <button hidden data-drawer-target="filters-drawer" data-drawer-show="filters-drawer" data-drawer-placement="top"
    aria-controls="filters-drawer"></button>
  ...
  <div id="filters-drawer"
    ...
    <div class="flex flex-col items-center my-5">
      ...
      <ul class="grid w-full md:w-1/2 lg:w-1/3 gap-6 sm:grid-cols-2">
        <li class="px-10 md:px-0">
          <input type="radio" id="feed-explore" name="feed" value="explore" class="hidden peer" required
            :checked="!data.getFollowedPosts" @click="feedChangeRequested()">
          <label for="feed-explore"
        ...
        <li class="px-10 md:px-0">
          <input type="radio" id="feed-following" name="feed" value="following" class="hidden peer"
            :checked="data.getFollowedPosts" @click="feedChangeRequested()">
          <label for="feed-following"
            class="inline-flex items-center justify-between w-full p-4 text-gray-500 bg-white border border-violet-200 rounded-xl cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 dark:peer-checked:text-violet-600 peer-checked:border-violet-600 peer-checked:text-violet-600 hover:text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
    ...
    <div class="w-full sm:w-1/2 lg:w-1/3 px-10 sm:px-0 sm:mx-auto mb-5">
      ...
      <div class="relative">
        ...
        <input @input="findCompanies($event.target.value)" type="search" id="search-company"
          ...
          placeholder="Trova aziende ..." required>
      </div>
      <ul class="min-h-[15vh] max-h-[15vh] mt-4 overflow-y-auto bg-gray-50 rounded-xl bg-opacity-50 dark:bg-gray-700">
        <li v-for="company in data.companies" v-if="!data.loadingCompanies">
          <a class="flex items-center justify-center my-1" href="./">
            <UserCircleIcon v-if="company.profilePicture === null || company.profilePicture === ''"
              class="w-1/3 h-10 text-gray-900 dark:text-gray-50" />
            <img v-else class="w-1/2 lg:w-1/3 h-10 overflow-hidden" :src="company.profilePicture"
              alt="user-profile-picture" />
            <span class="py-auto w-1/2 lg:w-1/3 text-gray-900 dark:text-gray-50 align-middle">{{ company.companyName
            }}</span>
          </a>
        </li>
        <li class="flex items-center justify-center my-1 animate-pulse" role="status" v-else>
          <svg class="w-1/2 lg:w-1/3 h-10 text-gray-200 dark:text-gray-700" aria-hidden="true" fill="currentColor"
    ...
    <div class="p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700" data-drawer-hide="filters-drawer">
      <span
        class="absolute bottom-3 w-8 h-1 -translate-x-1/2 bg-violet-300 rounded-lg left-1/2 dark:bg-violet-600"></span>
    </div>
  </div>
</template>
