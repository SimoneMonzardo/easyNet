<template>
  ...
  <div class="max-h-[calc(100vh-4rem)] w-screen grid grid-cols-8 grid-rows-[8]">
    ...
    <div v-if="pending || data.status !== 200" class="col-start-2 row-start-2 col-end-[8] row-end-[8] flex-col justify-center h-full w-full">
    ...
    <PostsFeedSection :post="data.posts[data.activePost]" v-else class="col-start-2 row-start-2 col-end-[8] row-end-[8]"/>

    <button class="col-start-8 row-start-[8] rotate-180" @click="nextPost">
      <div class="block triangle"></div>
      <ChevronDoubleLeftIcon class="absolute inset-2 h-10 w-10 text-blue-500 rotate-45 bg-transparent" />
    </button>
  </div>
