<template>
  ...
  <div class="flex flex-col mt-10 gap-y-8">
    ...
    <ul
      ...
      class="px-4 grid sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-y-4 sm:gap-4 w-screen animation-pulse"
    >
      <li
    ...
    <ul
      v-else
      class="lg:mx-6 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4"
    >
      <li v-for="post in data.posts">
        <button
          type="button"
          @click="openHighlightModal(post)"
          class="border border-gray-100 rounded-xl p-2"
        >
          <div
            v-if="post.content.data.image === ''"
            v-html="post.content.content"
            class="mx-auto text-gray-900 dark:text-gray-50"
            :class="post.content.data.image === '' ? 'h-full' : ''"
          ></div>
          <img
            v-else
            :src="post.content.data.image"
            class="h-auto max-h-full rounded-lg mx-auto"
          />
        </button>
      </li>
    </ul>
