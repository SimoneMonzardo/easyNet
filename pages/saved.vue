<template>
  ...
  <div id="highlight-modal" tabindex="-1" class="bg-gray-900 bg-opacity-50 fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full">
   <div class="relative w-full max-w-4xl max-h-full">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="flex items-center justify-between p-6 pb-0 rounded-t dark:border-gray-600">
          <PostHeader
            class="w-full mr-1 sm:mr-5"
            :username="data.selectedPost.username"
            :elapsedTime="data.selectedPost.elapsed"
            :profilePicture="data.selectedPost.imageUrl"
        ...
        <div class="p-6 pt-3 space-y-3 w-full">
          ...
          <div class="w-full sm:w-3/4 md:w-1/2 mx-auto">
            <LikeCommentsButtons 
              class="h-10" 
              :likes="data.selectedPost.likesCount"
              :comments="data.selectedPost.commentsCount" 
              :hasUserLike="data.selectedPost.hasUserLike"
              :isSavedByUser="data.selectedPost.isSavedByUser" 
   ...
   <div class="flex flex-col mt-10 gap-y-8">
    <h1 class="mx-auto mb-4 text-4xl font-extrabold tracking-tight leading-none md:text-5xl lg:text-6xl text-violet-600">Post salvati</h1>
    <ul v-if="pending || data.status !== 200" class="px-4 grid sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-y-4 sm:gap-4 w-screen animation-pulse">
...
const { pending } = useFetch('https://progettoeasynet.azurewebsites.net/Save/GetSavedPosts', {
  lazy: true,
  server: false,
  method: 'GET',
  headers: {
...
function openHighlightModal(post) {
  ...
  data.selectedPost.commentsCount = formatNumber(data.selectedPost.comments.length);
...
async function toggleLike() {
  ...
  await useFetch(`https://progettoeasynet.azurewebsites.net/Like/PostLike?postId=${data.selectedPost.postId}`, {
    lazy: true,
    server: false,
    method: 'POST',
    headers: {
...
async function toggleSave() {
  ...
  await useFetch(`https://progettoeasynet.azurewebsites.net/Save/PostSave?postId=${data.selectedPost.postId}`, {
    lazy: true,
    server: false,
    method: 'POST',
    headers: {
