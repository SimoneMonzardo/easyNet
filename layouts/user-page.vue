  ...
  <NuxtLayout name="default">
    <div class="flex justify-center w-[75vw] max-h-[calc(100vh-4rem)]">
      <div v-if="userBehavior">
        ...
        <div class="flex xl:max-w-full max-h-[calc(100%-25rem)] sm:max-h-[calc(100%-15rem)] overflow-y-auto">
      ...
      <div v-else>
        <ErrorTemplate :error="errorContent" />
      </div>
    </div>
  </NuxtLayout>
</template>
...
const {
  data: userBehavior,
  pending,
} = await useFetch<UserBehavior>(`${config.public.apiUrl}/User/GetUserData`, {
  lazy: true,
  server: false,
